...
public class LazyTalonBuilder implements LazyCTREBuilder<LazyTalon, FeedbackSensorSourceValue>{
    ...
    /**
     * Constructs a LazyTalon instance with specific motor identifiers 
     * and configuration parameters
     * 
     * @param motorID the ID of the primary motor controller
     * @param sensorToMechanismRatio the gear ratio of the sensor to the mechanism 
     * 
     * @param invertedValue what direction that is positive to the motor
     * @param supplyCurrentLimit the current limit for the motor supply
     * @param statorCurrentLimit the current limit for the motor stator
     */
    ...
    @Override
    public LazyTalonBuilder withFollower(int followID, String canBus, MotorAlignmentValue followerInverted) {
    ...
    public LazyTalonBuilder withCANCoder(int CANCoderID, String canCoderCanBus, 
            FeedbackSensorSourceValue sensorType,
            double magnetOffset, 
            SensorDirectionValue sensorDirection, double discontinuityPoint,
